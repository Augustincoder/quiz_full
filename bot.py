...
# ⚠️ MUHIM O'ZGARISH: Bot yonganda bazadagi rasmiy testlarni yuklab olamiz
...
def invalidate_blocks_cache(subject_key: str | None = None):
    if subject_key: keys = [k for k in _blocks_kb_cache if k[0] == subject_key]
    else: keys = list(_blocks_kb_cache)
    for k in keys: _blocks_kb_cache.pop(k, None)

def get_blocks_keyboard(subject_key: str, page: int = 0) -> InlineKeyboardMarkup:
    cache_key = (subject_key, page)
    if cache_key in _blocks_kb_cache: return _blocks_kb_cache[cache_key]
...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    if chat_id in active_tests:
        ...
        if old_poll_id: poll_chat_map.pop(old_poll_id, None)
    ...
    if len(args) > 1:
        if args[1].startswith("s_"):
            ...
            if not test_data_db: return await message.answer("❌ Bu fan topilmadi yoki muallif tomonidan o'chirilgan.")
            ...
            if test_data_db: return await start_ugc_test(message, test_data_db, bot)
            else: return await message.answer("❌ Bu blok topilmadi yoki muallif tomonidan o'chirilgan.")

    await message.answer("🏛 *Talabalar Imtihon Trenajyori*\n\nAssalomu alaykum! Kerakli bo'limni tanlang:", reply_markup=get_main_keyboard(), parse_mode="Markdown")
...
async def show_ugc_subject_blocks(message: Message, creator_id: str, subject: str):
    ...
    if not subj_tests: return await message.answer("❌ Bu fanda bloklar topilmadi.")

    buttons = [[InlineKeyboardButton(text=f"📘 {t['block_name']}", callback_data=f"ugc_start_{t['id']}")] for t in subj_tests]
    buttons.append([InlineKeyboardButton(text="🏠 Asosiy Menyu", callback_data="back_to_main")])
    await message.answer(f"📚 *Fan:* {subject}\n\nQuyidagi test bloklaridan birini tanlang:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="Markdown")
...
async def create_test_start(callback: CallbackQuery, state: FSMContext):
    ...
    subjects = {}
    for t in tests: subjects.setdefault(t["subject"], t["id"])

    buttons = [[InlineKeyboardButton(text=f"📁 {subj}", callback_data=f"ct_exist_{ref_id}")] for subj, ref_id in subjects.items()]
...
async def ct_new_subject(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("👉 *Yangi fanning nomini kiriting:*\n_(Masalan: Anatomiya, Fizika 1-qism)_", reply_markup=kb, parse_mode="Markdown")
...
async def ct_exist_subject(callback: CallbackQuery, state: FSMContext):
    ...
    if not test_data: return await callback.answer("Xatolik! Fan topilmadi.", show_alert=True)
    await state.update_data(subject=test_data["subject"])
    await state.set_state(CreateTestStates.waiting_for_name)
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Bekor qilish", callback_data="cancel_creation")]])
    await callback.message.edit_text(f"✅ Fan tanlandi: *{test_data['subject']}*\n\n👉 Endi ushbu fan uchun *yangi blok nomini* kiriting:\n_(Masalan: 1-Mavzu, Yakuniy imtihon)_", reply_markup=kb, parse_mode="Markdown")
...
async def create_test_subject(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Bekor qilish", callback_data="cancel_creation")]])
    await message.answer(f"✅ Fan yaratildi: *{subject}*\n\n👉 Endi ushbu fan ichidagi *birinchi blok uchun nom* bering:\n_(Masalan: 1-Mavzu, 1-Blok)_", reply_markup=kb, parse_mode="Markdown")
    await state.set_state(CreateTestStates.waiting_for_name)
...
async def receive_docx_file(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await msg.edit_text(f"✅ Fayl o'qildi! *{len(new_qs)} ta* savol topildi. Jami: {len(questions)} ta.\n\nYana fayl yuboring yoki Yakunlashni bosing.", reply_markup=kb, parse_mode="Markdown")
    except Exception:
...
async def receive_question(message: Message, state: FSMContext):
    ...
    if fmt == "quiz":
        if not message.poll or message.poll.type != "quiz": return await message.answer("⚠️ Iltimos, Telegram Quiz yuboring!")
        questions.append({
            "question": message.poll.question, "options": [o.text for o in message.poll.options], "correct_index": message.poll.correct_option_id,
        })
    elif fmt == "text":
        if not message.text: return await message.answer("⚠️ Iltimos, matn yuboring!")
        added = _parse_text_questions(message.text)
        if not added: return await message.answer("⚠️ Xato! To'g'ri javob oldiga # qo'yishni unutmang.")
...
async def finish_creation(callback: CallbackQuery, state: FSMContext, bot: Bot):
    data = await state.get_data()
    questions = data.get("questions", [])
    if not questions: return await callback.answer("Hech qanday savol yo'q!", show_alert=True)
    ...
    text = (f"🎉 *Blok muvaffaqiyatli saqlandi!*\n\n📚 Fan: {data['subject']}\n📝 Blok: {data['block_name']}\n🔢 Savollar: {len(questions)} ta\n\n🔗 *Faqat shu blokka to'g'ridan-to'g'ri kirish uchun havola:*\n`{link}`\n\n_(Mening Testlarim bo'limiga kirsangiz, butun boshli Fanni ulashish ssilkasini ham olishingiz mumkin)_")
...
async def my_tests_handler(callback: CallbackQuery):
    ...
    for t in tests: subjects.setdefault(t["subject"], []).append(t)

    buttons = [[InlineKeyboardButton(text=f"📁 {subj} ({len(subj_tests)} ta blok)", callback_data=f"manage_subj_{subj_tests[0]['id']}")] for subj, subj_tests in subjects.items()]
    buttons.append([InlineKeyboardButton(text="🔙 Asosiy Menyu", callback_data="back_to_main")])
...
async def manage_subj_handler(callback: CallbackQuery, bot: Bot):
    ...
    text = (f"📚 *Fan:* {test_data['subject']}\n\n🔗 *Ushbu fanni to'liq ulashish (Ssilka):*\n`{link}`\n_(Do'stlaringiz shu orqali kirsa, pastdagi barcha bloklarni ko'ra oladi)_\n\nBoshqarish uchun blokni tanlang:")
    buttons = [[InlineKeyboardButton(text=f"🔖 {t['block_name']}", callback_data=f"manage_test_{t['id']}")] for t in subj_tests]
...
async def manage_test_handler(callback: CallbackQuery):
    ...
    if not test_data or str(test_data["creator_id"]) != str(callback.from_user.id): return await callback.answer("Test topilmadi!", show_alert=True)

    text = (f"📝 *Blok Ma'lumotlari*\n\n📚 Fan: {test_data['subject']}\n🔖 Blok: *{test_data['block_name']}*\n🔢 Savollar: {len(test_data['questions'])} ta\n📅 Yaratilgan: {test_data['created_at'][:10]}")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def delete_test_handler(callback: CallbackQuery):
    ...
    else: await callback.answer("❌ Xatolik yuz berdi.", show_alert=True)

async def start_ugc_test(message: Message, test_data_db: dict, bot: Bot):
    chat_id = message.chat.id
    if chat_id in active_tests or chat_id in waiting_rooms: return await message.answer("⚠️ Bu chatda tugallanmagan test bor. /stop yozing.")

    session_q = prepare_shuffled_questions(test_data_db["questions"])
    active_tests[chat_id] = {
        "chat_type": "private", "initiator_id": message.from_user.id, "subject_key": test_data_db["subject"],
        "test_id": f"ugc_{test_data_db['id']}", "block_name": test_data_db.get("block_name", ""),
...
async def show_official_tests(callback: CallbackQuery):
    buttons = [[InlineKeyboardButton(text=f"📘 {subj_name} ({len(memory_db.get(subj_key, {}))} ta blok)", callback_data=f"subj_{subj_key}")] for subj_key, subj_name in SUBJECTS.items()]
    buttons.append([InlineKeyboardButton(text="🔙 Asosiy Menyu", callback_data="back_to_main")])
...
async def admin_users_list_paginated(callback: CallbackQuery):
    ...
    lines = [f"*{i}.* [{u.get('full_name') or 'Ismsiz'}](tg://user?id={u.get('telegram_id')}){_uname(u)} | 📅 {u.get('joined_at', '')[:10]}" for i, u in enumerate(current_users, start_idx + 1)]
    text = f"👥 *Barcha foydalanuvchilar ({page+1}/{total_pages}):*\n\n" + "\n".join(lines)

    buttons, nav = [], []
    if page > 0: nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"admin_users_page_{page-1}"))
    if page < total_pages - 1: nav.append(InlineKeyboardButton(text="➡️", callback_data=f"admin_users_page_{page+1}"))
    if nav: buttons.append(nav)
    buttons.append([InlineKeyboardButton(text="🔙 Admin panel", callback_data="admin_panel_main")])

    try: await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="Markdown")
    except Exception: pass
    await callback.answer()
...
def _save_official_test(subj: str, t_id: int, questions: list):
    """Rasmiy testlarni Supabasega va memory_db ga saqlash."""
...
def _admin_controls_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Matn", callback_data="adm_switch_text"), InlineKeyboardButton(text="📄 Word (.docx)", callback_data="adm_switch_docx")],
...
async def _admin_show_input_prompt(message: Message, fmt: str, total: int, edit: bool = False):
    ...
    hint = ("Savollarni ushbu qolipda yuboring (bir xabarda ko'plab savollar mumkin):\n\n`Savol matni?\n#To'g'ri javob\nXato javob 1\nXato javob 2`\n\n_Savollar orasida bo'sh qator bo'lsin._") if fmt == "text" else ("`.docx` fayl yuboring. Tuzilishi:\n\n`Savol matni\n#To'g'ri javob\nXato javob\nXato javob`\n\n_Savollar orasida bo'sh qator bo'lsin._\n_(Bir nechta fayl ketma-ket yuborishingiz mumkin)_")
    bar = "▓" * min(total, 20) + "░" * max(0, 20 - total) if total <= 20 else "▓" * 20
    text = (f"➕ *Rasmiy test qo'shish*\n\n📌 Joriy format: {fmt_label}\n📊 Yig'ilgan savollar: *{total} ta* {bar}\n\n{hint}\n\n💡 Formatni o'zgartirish, ko'rish yoki saqlash uchun pastdagi tugmalardan foydalaning.")
    if edit:
        try: await message.edit_text(text, reply_markup=_admin_controls_kb(), parse_mode="Markdown")
        except Exception: await message.answer(text, reply_markup=_admin_controls_kb(), parse_mode="Markdown")
    else: await message.answer(text, reply_markup=_admin_controls_kb(), parse_mode="Markdown")
...
async def admin_add_test_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📂 Qaysi fanga rasmiy test qo'shmoqchi (yoki tahrirlamoqchi)siz?", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def admin_add_test_subj(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔢 *Blok raqamini (ID) kiriting:*\n_(Masalan: 1, 2, 3... Agar bu raqamli test avvaldan bor bo'lsa, u yangilanadi)_", parse_mode="Markdown")
...
async def admin_add_test_id(message: Message, state: FSMContext):
    if not message.text.isdigit(): return await message.answer("⚠️ Iltimos, faqat raqam kiriting (Masalan: 1)")
...
async def admin_preview(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    questions = data.get("questions", [])
    if not questions: return await callback.answer("Hali hech qanday savol yo'q!", show_alert=True)
...
async def admin_receive_text(message: Message, state: FSMContext):
    data = await state.get_data()
    if data.get("format") != "text": return await message.answer("⚠️ Hozir *Word* formati tanlangan.\nMatn yubormoqchi bo'lsangiz, *📝 Matn* tugmasini bosing.", parse_mode="Markdown")
    new_qs = _parse_text_questions(message.text)
    if not new_qs: return await message.answer("⚠️ Savol topilmadi!\n\nFormat:\n`Savol?\n#To'g'ri javob\nXato 1\nXato 2`\n\n_To'g'ri javob oldiga # qo'yishni unutmang._", parse_mode="Markdown")
    questions = data.get("questions", []) + new_qs
    await state.update_data(questions=questions)
...
async def admin_receive_docx(message: Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    if data.get("format") != "docx": return await message.answer("⚠️ Hozir *Matn* formati tanlangan.\nWord fayl yubormoqchi bo'lsangiz, *📄 Word* tugmasini bosing.", parse_mode="Markdown")
    if not message.document.file_name.endswith(".docx"): return await message.answer("⚠️ Faqat `.docx` kengaytmali fayl qabul qilinadi.")
    ...
    try:
        ...
        await bot.download_file(file.file_path, file_path)
        new_qs = _parse_docx_questions(file_path)
        if not new_qs: return await msg.edit_text("❌ Fayldan savol topilmadi.\n\nFayl tuzilishi:\n`Savol matni\n#To'g'ri javob\nXato javob\nXato javob`\n_Savollar orasida bo'sh qator bo'lsin._", parse_mode="Markdown")
        questions = data.get("questions", []) + new_qs
        await state.update_data(questions=questions)
        await msg.delete()
...
async def admin_finish_creation(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    questions = data.get("questions", [])
    if not questions: return await callback.answer("⚠️ Hali hech qanday savol yo'q!", show_alert=True)

    subj = data["subject"]
    t_id = data["test_id"]
    try:
        _save_official_test(subj, t_id, questions)
        await callback.message.edit_text(f"✅ *Rasmiy test saqlandi!*\n\n📚 Fan: *{SUBJECTS.get(subj, subj)}*\n🔖 Blok ID: *{t_id}*\n🔢 Savollar soni: *{len(questions)} ta*", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await state.clear()
    except Exception as e: await callback.answer(f"❌ Xatolik: {e}", show_alert=True)
    await callback.answer()
...
async def show_leaderboard_handler(callback: CallbackQuery, bot: Bot):
    ...
    if _leaderboard_cache["text"] and now - _leaderboard_cache["ts"] < _LEADERBOARD_TTL:
        await callback.message.edit_text(_leaderboard_cache["text"], reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Asosiy Menyu", callback_data="back_to_main")]]), parse_mode="Markdown")
    ...
    if not top_users: text = "🏆 *GLOBAL REYTING*\n\nHozircha reytingda hech kim yo'q."
        ...
        async def _get_name(user_id):
            ...
            except Exception: return "Sirli Talaba"

        names = await asyncio.gather(*[_get_name(u["user_id"]) for u in top_users])

        lines = [f"{medals[i] if i < 10 else '🔸'} *{name}*\n      ✅ {user['correct']} ta to'g'ri | 📝 {user['completed']} ta test" for i, (user, name) in enumerate(zip(top_users, names))]
...
async def show_history_detail(callback: CallbackQuery):
    ...
    if not item.get("mistakes"): parts.append("🎉 *Xato qilinmagan!*")
    else:
        parts.append("📑 *XATOLAR:*\n")
        for i, m in enumerate(item["mistakes"], 1): parts.append(f"*{i}.* {m['question']}\n❌ {m['wrong_ans']}\n✅ {m['correct_ans']}")
    ...
    if session["chat_type"] == "private":
        ...
        async with lock:
            ...
            if is_correct: score["correct"] += 1
                ...
                score["wrong"] += 1
                score["mistakes"].append({"question": q_data["question"], "correct_ans": q_data["correct_text"], "wrong_ans": q_data["options"][poll_answer.option_ids[0]]})
...
async def finish_test(chat_id: int, bot: Bot):
    ...
    if session["chat_type"] == "private":
        ...
        if str(t_id).startswith("ugc_"): buttons.append([InlineKeyboardButton(text="🔁 Qayta ishlash", callback_data=f"ugc_start_{str(t_id).replace('ugc_', '')}")])
        elif t_id != "mock":
        ...
        await asyncio.gather(*[asyncio.to_thread(stats_manager.update_user_stats, u_id, scores["correct"], scores["wrong"], session["subject_key"], session["test_id"], scores["mistakes"]) for u_id, scores in session["group_scores"].items()])
        if not session["group_scores"]: body = "Hech kim qatnashmadi 😔"
        else:
            medals = ["🥇", "🥈", "🥉"]
            sorted_scores = sorted(session["group_scores"].values(), key=lambda x: x["correct"], reverse=True)
            body = "\n".join(f"{medals[i] if i < 3 else '🔸'} {s['name']}: {s['correct']} ta to'g'ri" for i, s in enumerate(sorted_scores))
        text = f"🏁 *{title} yakunlandi!*\n⏱ Vaqt: {mins:02d}:{secs:02d}\n\n🏆 *NATIJALAR:*\n{body}"

    if str(t_id).startswith("ugc_"): buttons.append([InlineKeyboardButton(text="🏠 Asosiy Menyu", callback_data="post_main")])
    else: buttons.extend([[InlineKeyboardButton(text="🔙 Fan menyusiga", callback_data=f"post_subj_{session['subject_key']}")], [InlineKeyboardButton(text="🏠 Asosiy Menyu", callback_data="post_main")]])
...
async def review_mistakes_handler(callback: CallbackQuery):
    ...
    parts.extend(f"*{i}.* {m['question']}\n❌ {m['wrong_ans']}\n✅ {m['correct_ans']}" for i, m in enumerate(mistakes, 1))
    text = "\n\n".join(parts)
    if len(text) > 4000: text = text[:4000] + "\n... (kesildi)."
